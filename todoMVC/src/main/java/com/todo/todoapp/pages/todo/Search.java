...
public class Search {
    ...
    public void init() {
        ...
        List<Todo> list = todoService.findWithNamedQuery(Todo.BY_USER, QueryParameters.with("user", loggedUser.getId()).parameters());
       
        //Matching input string against search query
        
        String pattern = ".*" + query + ".*";
        Pattern regex = Pattern.compile(pattern, Pattern.CASE_INSENSITIVE);
        todoList = new LinkedList<Todo>();
        for(Todo todo : list){
        	Matcher m = regex.matcher(todo.getTitle());
        	if(m.matches()){
        		todoList.add(todo);
        	}
        }
